...
- (BOOL)trainModel
{
    ...
    if (images.size() > 0 && labels.size() > 0) {
        //test codes
        /*double tv= _model->getDouble("threshold");
        NSLog(@"Threshold %lf",tv);*/
        //int tv= _model->getInt("num_components");
        //NSLog(@"NUM COMPONENTS---> %d",tv);
...
- (NSDictionary *)recognizeFace:(cv::Rect)face inImage:(cv::Mat&)image
{
    ...
    NSLog(@"PredictLebel %d and Confidence %lf",predictedLabel,confidence);
